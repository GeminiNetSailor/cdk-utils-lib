...
export class CoreCdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: AwsCdkStackProps) {
    super(scope, `${id}-web-api-cdk-stack`, props);

    var pipelineNameId = `${id}-pipeline`;
    const pipeline = new cdk.pipelines.CodePipeline(this, pipelineNameId, {
    ...
    pipeline.addStage(new RestApiStage(this, id, { branch: props.branch }));
  }
